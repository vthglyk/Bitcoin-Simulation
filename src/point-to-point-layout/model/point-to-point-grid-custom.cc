...
#include "ns3/point-to-point-grid-custom.h"
...
NS_LOG_COMPONENT_DEFINE ("PointToPointGridHelperCustom");

PointToPointGridHelperCustom::PointToPointGridHelperCustom (uint32_t nRows, 
                                                uint32_t nCols, int noCpus,
                                                PointToPointHelper pointToPoint)
  : m_xSize (nCols), m_ySize (nRows), m_noCpus(noCpus)
{
  // Bounds check
      ...
      for (uint32_t x = 0; x < nCols; ++x)
        {
          ...
          //std::cout << "Creating a node with Id = " << y*nCols+x << "\n";
...
PointToPointGridHelperCustom::~PointToPointGridHelperCustom ()
{
}

void
PointToPointGridHelperCustom::InstallStack (InternetStackHelper stack)
{
...
void
PointToPointGridHelperCustom::AssignIpv4Addresses (Ipv4AddressHelper rowIp, Ipv4AddressHelper colIp)
{
...
void
PointToPointGridHelperCustom::AssignIpv6Addresses(Ipv6Address addrBase, Ipv6Prefix prefix)
{
...
void
PointToPointGridHelperCustom::BoundingBox (double ulx, double uly,
...
Ptr<Node> 
PointToPointGridHelperCustom::GetNode (uint32_t row, uint32_t col)
{
  if (row > m_nodes.size () - 1 || 
      col > m_nodes.at (row).GetN () - 1) 
    {
      NS_FATAL_ERROR ("Index out of bounds in PointToPointGridHelperCustom::GetNode.");
    }
...
Ipv4Address
PointToPointGridHelperCustom::GetIpv4Address (uint32_t row, uint32_t col)
{
  if (row > m_nodes.size () - 1 || 
      col > m_nodes.at (row).GetN () - 1) 
    {
      NS_FATAL_ERROR ("Index out of bounds in PointToPointGridHelperCustom::GetIpv4Address.");
    }
...
Ipv6Address
PointToPointGridHelperCustom::GetIpv6Address (uint32_t row, uint32_t col)
{
  if (row > m_nodes.size () - 1 ||
      col > m_nodes.at (row).GetN () - 1)
    {
