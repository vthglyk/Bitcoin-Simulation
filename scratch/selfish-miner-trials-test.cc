...
main (int argc, char *argv[])
{
  ...
  //LogComponentEnable("BitcoinSelfishMinerTrials", LOG_LEVEL_DEBUG);
  ...
  cmd.AddValue ("step", "Step of the percentiles", step);
...
/*         std::cout << "SystemId " << systemId << ": Miner " << miner.first << " with hash power = " << minersHash[count] 
    ...
    bitcoinMinerHelper.SetMinerType (SELFISH_MINER_TRIALS);
    ...
    std::cout << "Iteration " << iter+1 << " lasted " << tSimFinish - tSimStart << "s: " << stats[1].attackSuccess << " trials.\n\n";
    trials.push_back(stats[1].attackSuccess);
  }
  ...
  if (systemId == 0)
  {
    ...
    int percentilesSize = 1/step;
    double *percentiles = new double[percentilesSize];
	
    std::cout << "The step was = " << step << ".\n";

    for(int i = 0; i < percentilesSize - 1; i++)
    {
      percentiles[i] = *(trials.begin()+int(trials.size()*(i+1)*step));
      std::cout << (i+1)*step*100 << "% percentile = " << percentiles[i] << "\n";
    }
	
    percentiles[percentilesSize - 1] = trials.back();
    std::cout << "100% percentile = " << percentiles[percentilesSize - 1] << "\n";
